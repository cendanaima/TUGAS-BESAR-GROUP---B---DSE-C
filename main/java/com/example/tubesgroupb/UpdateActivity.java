...
public class UpdateActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String getinfo = getIntent().getStringExtra("nama");
        String[] pecah = getinfo.split("-");
        cursor = db.rawQuery("SELECT * FROM pegawai WHERE nama = '" + pecah[1] + "'", null);
        cursor.moveToFirst();
        ...
        ton1.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View arg0) {
                ...
                if (edit.isEmpty()) {
                    ...
                    db.execSQL("update pegawai set nama='" +
                            text2.getText().toString() + "', tgl='" + text3.getText().toString() + "', jk='" + text4.getText().toString() + "', alamat='" +
                            text5.getText().toString() + "' where nip='" + text1.getText().toString() + "'");
                    Toast.makeText(getApplicationContext(), "Perubahan Tersimpan...", Toast.LENGTH_LONG).show();
                    HomeActivity.da.RefreshList();
                    finish();
                }
            }
        });
